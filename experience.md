---
layout: page
title: '💼 Professional Experience'
permalink: /experience/
---

<hr style="border: 1px solid #ccc; margin: 2rem 0;">

<style>
  .experience-card {
    display: flex;
    align-items: flex-start;
    margin-bottom: 2rem;
    gap: 1.5rem;
  }
  .experience-card img {
    width: 140px;
    max-width: 100%
    height: auto;
    border-radius: 5px;
    object-fit: cover;
    box-shadow: 0 2px 6px rgba(0,0,0,0.15);
  }
</style>

## ⚙️ Research and Technical Roles


<div class='experience-card'>
    <img src='/assets/img/sdsuai4bus.png' alt='AI4Business SDSU'>
    <div>
        <h3> Research Assistant </h3>
        <p><strong>SDSU AI4Business Lab</strong> • August 2025 - Present</p>
        <ul>
            <li>Contribute to the development of <i>ENV-QAS</i>, a multimodal AI framework informing real-time disaster management, leveraging NLP, multimodal transformers,  and temporal knowledge graphs to generate actionable insights from diverse sources </li>
            <li> Collaborate and communicate with research faculty and lab members to implement end-to-end pipeline for query understanding, environmental context extraction, and recommendation generation for crisis response.</li>
            <li>Analyze and and synthesize AI/ML research papers and technical documentation to extract relevant methodologies and inform design decisions </li>
        </ul>
    </div>
</div>

<div class='experience-card'>
    <img src='/assets/img/bttlogo.png' alt='Break Through Tech Logo'>
    <div>
        <h3> AI Studio Fellow </h3>
        <p> <a href='https://www.breakthroughtech.org/'> Break Through Tech </a> • April 2025 - Present </p>
        <p><em>Python, sklearn, Keras, NLP, Computer Vision </em></p>
        <ul>
            <li> Completed Cornell Tech ML Foundations certificate, using industry relevant tools and Python libraries </li>
            <li>Collaborate with team of fellows on AI challenge project (Synthetic Image Data Generation) with <i> Anote</i>, an NYC AI Technology Startup with mentorship and support from industry professionals </li>
            <li> Further leadership and communication skills and learn to responsibly and ethically use AI / ML </li>
        </ul>
    </div>
</div>

<div class='experience-card'>
    <img src='/assets/img/vera-rubin-logo.png' alt='Vera Rubin Obs. Logo'>
    <div>
        <h3> Undergraduate Researcher </h3>
        <p> <strong> SDSU Department of Astronomy </strong> • May 2025 - August 2025 </p>
        <p><em>Python, sklearn, pandas, numpy, matplotlib, astropy, Git</em></p>
        <ul>
            <li> Apply ML and data science techniques to identify and analyze pulsating variable stars using Vera Rubin Observatory DP1 Data </li>
            <li> Collaborate on cross disciplinary team of undergrads under mentorship of Dr. Eric Sandquist </li>
            <li>Communicate plots and findings in scientific presentation and potential publication </li>
            <li> Access GitHub repo <a href='https://github.com/McAelanRemigio/VeraRubin' target='_blank'> here </a> </li>
        </ul>
    </div>
</div>


<div class='experience-card'>
    <img src='/assets/img/drone-sim.jpg' alt='Drone Simulation'>
    <div>
        <h3> Research Assistant </h3>
        <p> <strong>SDSU Dept. of Computer Science</strong> • February 2025 - May 2025 </p>
        <p> <em>Linux, CLI, Embedded Systems, Gazebo</em> </p>
        <ul>
            <li> Assisted Professor Umut Can Cabuk in research focused on autonomous drone systems </li>
            <li> Conducted Software in the Loop (SIL) drone simulations and testing via Gazebo and qGroundControl </li>
            <li> Gained hands-on experience with Ubuntu CLI and embedded systems tools </li>
        </ul>
    </div>
</div>

<hr style="border: 1px solid #ccc; margin: 2rem 0;">

## 🎓 Teaching & Mentorship

<div class='experience-card'>
    <img src='/assets/img/reo.png' alt='STAR Center Logo'>
    <div>
        <h3> STAR Center Tutor </h3>
        <p> <strong> SDSU Residential Education Office </strong> • September 2024 - May 2025 </p>
        <ul>
            <li> Facilitated 4-5 daily tutoring sessions for on-campus residents in Calculus I/II, programming, and physics, customized to different learning goals  </li>
            <li>Adapted explanations to diverse learning stlyes and paces, contributing to noticeable gains in student confidence and problem-solving  </li>
            <li>Supported multiple students who achieved midterm score improvements of <b>10-20%</b> + final course grades of <b>A or A-</b>  </li>
        </ul>
    </div>
</div>
